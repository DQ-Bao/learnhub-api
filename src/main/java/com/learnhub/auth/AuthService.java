...
public class AuthService {
    ...
    public void forgotPassword(ForgotPasswordRequest req) {
        User user = userRepository.findByEmail(req.email())
            .orElseThrow(() -> new UserNotFoundException("Account with this email not found."));
    ...
    public void resetPassword(ResetPasswordRequest req) {
        ...
        User user = userRepository.findByEmail(email).orElseThrow();
        if (jwtService.isTokenExpired(req.token())) {
            emailService.sendForgotPasswordEmail(user.getEmail(), jwtService.generateToken(user, 30 * 60 * 1000));
            throw new ExpiredTokenException("Link is expired. Check email.");
        }
        String encoded = passwordEncoder.encode(req.password());
